...
public class AICarRoad : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        Point = GameObject.Find("PointRoad" + CurrentPoint);    //the script will search the next point automatically
        ...
        if (gameObject.name.Length == 17)//one-digit number name has 13 characters (for example: TrackerAICar2)
        {
            AICarName = gameObject.name.Substring(gameObject.name.Length - 6);//get the last 6 characters (for example: AICar2)
        }
        if (gameObject.name.Length == 18)//two-digit numbers name has 14 characters (for example: TrackerAICar24)
        {
    ...
    IEnumerator OnTriggerEnter(Collider collision)
    {
        if (collision.gameObject.name == AICarName)//If the AICar collider is detected
        {
            ...
            if (GameObject.Find("PointRoad" + CurrentPoint) == null)//if the next point doesn't exists, that means that the last point was passed
            {
