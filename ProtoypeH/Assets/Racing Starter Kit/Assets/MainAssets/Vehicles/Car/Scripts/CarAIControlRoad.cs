...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class CarAIControlRoad : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            {
                TargetGameObject = GameObject.Find("RoadTrackerAICar" + gameObject.name.Substring(gameObject.name.Length - 1));
            }
            if (gameObject.name.Length == 7)//two-digit number AI car has 7 characters (for example: AICar14)
            {
                TargetGameObject = GameObject.Find("RoadTrackerAICar" + gameObject.name.Substring(gameObject.name.Length - 2));
            }
